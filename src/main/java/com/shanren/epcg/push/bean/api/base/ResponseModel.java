package com.shanren.epcg.push.bean.api.base;
...
public class ResponseModel<M> implements Serializable {
    ...
    // 没有找到此患者信息
    public static final int ERROR_NOT_FOUND_PATIENT = 4044;
    ...
    // 请求参数错误-已存在患者
    public static final int ERROR_PARAMETERS_EXIST_PATIENT = 4004;
    // 请求参数错误-患者添加失败
    public static final int ERROR_PARAMETERS_ADD_PATIENT = 4005;

    // 请求参数错误-两次密码输入不一致
    public static final int ERROR_PASSWORD_NOT_SAME = 4006;
    // 服务器错误
    ...
    public static <M> ResponseModel<M> buildHavePatientError() {
        return new ResponseModel<M>(ERROR_PARAMETERS_EXIST_PATIENT, "已有此患者");
    }
    public static <M> ResponseModel<M> buildPatientAddError() {
        return new ResponseModel<M>(ERROR_PARAMETERS_ADD_PATIENT, "患者添加失败");
    }
    ...
    public static <M> ResponseModel<M> buildNotFoundPatientError(String str) {
        return new ResponseModel<M>(ERROR_NOT_FOUND_PATIENT, str != null ? str : "没有找到此患者.");
    }
    ...
    public static <M> ResponseModel<M> buildPasswordNotSameError() {
        return new ResponseModel<M>(ERROR_PASSWORD_NOT_SAME, "两次密码输入不一致");
    }
