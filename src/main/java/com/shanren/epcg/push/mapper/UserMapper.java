...
public interface UserMapper extends BaseMapper<User> {
    ...
    *//**
     ...
     * @return 返回一个用户
     *//*
    private static User createUser(String account,String password) {
        User user = new User();
        // 账户就是设备名
    ...
    *//**
     ...
     * @return List<User>
     *//*
    public static List<Patient> contacts(User self){
        return Hib.query(session -> {
            //重新加载一次用户信息到self中，和当前的session绑定
            session.load(self,self.getId());//懒加载导入

            //获取我关注的人
            Set<Record> flows = self.getRecords();
